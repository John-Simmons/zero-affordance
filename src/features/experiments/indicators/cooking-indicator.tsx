 ...
 * ## The 1s beat
 *
 * A scene only gets a third of the run — 600–1200ms out of the 1800–3600ms
 * band — so whatever it is doing has to fit inside that or it is cut off
 * partway through the gesture. The unit to fit is the BEAT: one complete
 * movement and back, which is not the same as one pass of the keyframes.
 * Figma emitted `pour-jug` and all three `bake-*` sets as two mirrored beats
 * inside a single loop (frame 50% is byte-identical to frame 0%), while
 * `pour-stream` and `stir-spoon` are one beat over the whole loop. Every set
 * arrived at 2s; the numbers on the utilities are where they ended up.
 *
 * So the durations differ on purpose, and the rule is the beat rather than the
 * loop — each set is timed by what one gesture of it is worth on screen, not by
 * a number shared across the file.
 *
 * The `bake-*` trio is the one place the two-beat structure is worth seeing
 * whole rather than just one beat of. Its two halves are not the same movement
 * twice: the oven rocks LEFT and settles, then RIGHT and settles. At 2s an act
 * showed 30–60% of that, so the right-hand rock essentially never played and
 * the oven read as leaning one way. At 1.25s it is 48–96%, about 72% at the
 * middle of the band, so the return rock lands rather than merely starting.
 *
 * That is 1.6x the speed it arrived at — close to the step that was too much
 * for `stir-spoon`, and it holds here because the movement is a small wobble
 * rather than a ±43° sweep: the same tempo reads as hurried on a big gesture
 * and as busy on a little one.
 *
 * All three sets move together because they are parts of one oven; retiming any
 * of them alone would slide the handle and the timer off the body they are
 * attached to.
 *
 * `stir-spoon` is what this is for. Its beat is a ±43° sweep out and back, and
 * at 2s a scene showed 30–60% of it — the spoon pushed away from the bowl and
 * the act ended before it ever came back, which read as a stall rather than as
 * stirring. It runs at 1.5s: 40–80% of the sweep, about 60% at the middle of
 * the band, so the spoon is on its way back by the time the act ends. It was
 * briefly 1s, which fits the whole sweep into an average act but stirs about
 * as fast as a whisk — the gesture being legible matters more here than the
 * gesture completing.
 *
 * `pour-stream` runs at 1s, and that number is not a judgement about pace: it
 * is the jug's. The stream and the jug are one action, `pour-jug` beats every
 * 1s, and the stream used to wobble on a 2s period against it — in step with
 * only every other tip. Matching them is what the duration is for, so this one
 * moves only if `pour-jug` does.
 *
 ...
 * happens to be when the third ends, and a short run gives each act less than a
 ...
 * top of the oven body, the stream on the bowl — and a translucent fill would
    ...
    >
      <g clipPath="url(#cooking-pour-clip)">
        ...
        <path
          ...
          fill="currentColor"
          className="origin-top-left animate-[pour-stream_1s_linear_infinite]"
        />
      </g>
      <defs>
      ...
      <g clipPath="url(#cooking-stir-clip)">
        ...
        */}
        ...
        <path
          ...
          fill="currentColor"
          className="origin-top-left animate-[stir-spoon_1.5s_linear_infinite]"
        />
      </g>
      <defs>
    ...
    >
      <g clipPath="url(#cooking-bake-clip)">
        ...
        <path
          ...
          fill="currentColor"
          className="origin-top-left animate-[bake-body_1.25s_linear_infinite]"
        />
        {/* Door handle */}
        <rect
          ...
          rx="100"
          fill="currentColor"
          className="origin-top-left animate-[bake-handle_1.25s_linear_infinite]"
        />
        {/* Timer */}
        <path
          ...
          fill="currentColor"
          className="origin-top-left animate-[bake-timer_1.25s_linear_infinite]"
        />
      </g>
      <defs>
